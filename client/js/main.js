...
constraints.video.facingMode = {
    ideal: "user" //전면카메라가 제일 이상적이다 안되면 후면으로 반환받음
}
...
function init() {
    ...
    socket.on('disconnect', () => {
        ...
        for (let socket_id in peers) { //배열 for in 하면 index로 나간다.
            removePeer(socket_id)
        }
    })
...
function removePeer(socket_id) {
    ...
    if (videoEl) {
        ...
        tracks.forEach(function (track) { //forEach() 주어진 함수를 배열 요소 각각에 대해 실행
            track.stop()
        })
    ...
    if (peers[socket_id]) peers[socket_id].destroy() //destroy와 remove??? 
...
function findAllIndex(string, char) { //! ?¨ª???????? arr??? ???????¨ª????????. 
    let arr = [];
